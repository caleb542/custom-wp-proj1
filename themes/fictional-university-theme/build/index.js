...
// import GoogMap from "./modules/GoogleMap"
...
class MyNotes {
  ...
  async deleteNote(e) {
    ...
    try {
      ...
      if (response.data.userNoteCount < response.data.userNoteLimit) {
        document.querySelector(".note-limit-message").classList.remove("active");
        // update count
        document.querySelector(".note-count-block small").innerHTML = `Total notes<br>${response.data.userNoteCount} of ${response.data.userNoteLimit}`;
      }
    } catch (e) {
      console.log("Sorry");
    }
    ...
    try {
      ...
      if (response.data != " You have reached your note limit.") {
        ...
        document.querySelector("#my-notes").insertAdjacentHTML("afterbegin", ` <li data-id="${response.data.id}" class="fade-in-calc">
            ...
            <button tabindex="0" class="edit-note"><i class="fa fa-pencil" aria-hidden="true"></i> Edit</button>
            <button tabindex="0" class="delete-note"><i class="fa fa-trash-o" aria-hidden="true"></i> Delete</button>
            <textarea readonly class="note-body-field">${response.data.content.raw}</textarea>
            <button tabindex="0" class="update-note btn btn--blue btn--small"><i class="fa fa-arrow-right" aria-hidden="true"></i> Save</button>
          </li>`);
...
class Search {
  ...
  getResults() {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default().getJSON(x, results => {
      this.resultsDiv.html(`
          <div class="row">
            <div class="one-third">
